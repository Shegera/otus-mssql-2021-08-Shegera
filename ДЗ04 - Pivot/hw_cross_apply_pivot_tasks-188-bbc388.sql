...
(
	SELECT
		SUBSTRING (
			REPLACE (Customers.CustomerName, ')', ''), CHARINDEX ('(', Customers.CustomerName) + 1, LEN (Customers.CustomerName)
		) AS CustomerName,
		Orders.OrderDate,
		1 AS x
	FROM
	...
	JOIN
		Sales.Orders WITH (NOLOCK) ON
		Orders.CustomerID = Customers.CustomerID
	WHERE
		Customers.CustomerID BETWEEN 2 AND 6
)
SELECT
	FORMAT (pvt.OrderDate, 'dd.MM.yyyy') AS InvoiceMonth,
	pvt.[Sylvanite, MT],
...
SELECT
	...
	StockItems.ExtendedPrice,
	StockItems.InvoiceDate
FROM
	Sales.Customers WITH (NOLOCK)
CROSS APPLY
	(
		...
		JOIN
			Warehouse.StockItems WITH (NOLOCK) ON
			StockItems.StockItemID = InvoiceLines.StockItemID
		WHERE
			Invoices.CustomerID = Customers.CustomerID
		ORDER BY
			InvoiceLines.ExtendedPrice DESC
	) AS StockItems
ORDER BY
	Customers.CustomerID,
	StockItems.StockItemID
